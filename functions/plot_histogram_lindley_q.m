function plot_lindley_q(samples, q_theta, x_max, q, numBins)
% plot_lindley_q
%
...
%
% INPUTS:
%   samples  - vector of data points sampled from q-Lindley distribution
...
% EXAMPLE USAGE:
%   samples = rand(1,500);
%   plot_lindley_q(samples, 2.5, 0.7, 15);
    ...
    % Define target q-Lindley distribution
    target_distribution = @(x) (q_theta^2 / (q_theta + 1)) .* ...
    ...
    xlabel('x');
    ylabel('Density');
    title('q-Lindley Distribution: Empirical vs Theoretical');
